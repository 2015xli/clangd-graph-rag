...
*   **Algorithm**:
    1.  **Span Loading**: The `SymbolEnricher` is invoked first. It parses the entire project with `tree-sitter` to find the precise body location of every function and enriches the in-memory `Symbol` objects with this `body_location` data.
