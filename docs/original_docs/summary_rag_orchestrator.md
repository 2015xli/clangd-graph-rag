...
It embodies the "worker-driven" architecture. It determines the *order* and *scope* of work but delegates the complex *logic* for processing each individual node to its worker functions and the `NodeSummarizer`.
...
1.  **Separation of Concerns**: The orchestrator handles logistics (DB I/O, parallelism, ordering) while the `NodeSummarizer` handles logic (staleness checks, LLM calls). This makes the system modular and easier to reason about.
...
2.  **Delegation (Logic)**: It makes a single call to the corresponding method on the `NodeSummarizer`, passing it the data it just fetched. It then waits for the processor to return a `status` and `data` packet.
