...
The core of the `NodeSummarizer` is a consistent, sequential decision-making pattern implemented in each of its `get_*_summary` methods. This "waterfall" logic ensures that work is done in the most efficient way possible, preferring cheaper, existing data sources before falling back to expensive LLM calls.
...
A key feature of the `NodeSummarizer` is its ability to handle contexts that exceed the LLM's token limit.
