...
*   **`NodeSummarizer` (The Logic Layer)**: The "brain" that performs the state-aware caching and generation logic for each node.
...
2.  **Pass 1: Targeted Code Analyses**
    ...
    *   **Logic**: The `NodeSummarizer` checks the code hash for each of these seed functions against the cache. Only functions with changed code will be regenerated, and their `code_analysis_changed` flag will be set in the `runtime_status`.
