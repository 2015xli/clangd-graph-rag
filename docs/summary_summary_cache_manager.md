...
1.  **Data Management**: Manages the in-memory cache for all summary-related data (`codeSummary`, final `summary`, and `code_hash`).
...
*   **Structure**: `Dict[str, Dict[str, Any]]`
    ...
    *   The value is a dictionary containing the cached data, for example: `{"code_hash": "...", "codeSummary": "...", "summary": "..."}`.
...
*   **Flags**: Each entry contains boolean flags:
    ...
    *   `code_summary_changed`: Set to `True` only when a `codeSummary` is truly regenerated by an LLM call. This flag is read by later passes to detect stale dependencies.
