...
4.  **Pass 4 & 5: `_summarize_all_files()` & `_summarize_all_folders()`**: After symbols are summarized, this triggers the roll-up summaries for all files, then all folders, and finally the project node.
5.  **Pass 6: `generate_embeddings()`**: The final pass queries for *all* nodes with a `summary` but no `summaryEmbedding` and generates the vectors.
...
### Pass 4 & 5: File and Folder "Roll-Up" Summaries
...
### Pass 6: Embedding Generation and State Management
