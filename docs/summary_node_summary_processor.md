...
*   `summary_restored` / `code_summary_restored`: A valid summary was found in the cache and should be written to the database to fill a gap. This does **not** trigger upstream dependency updates.
*   `summary_regenerated` / `code_summary_regenerated`: A new summary was successfully generated by the LLM. This **does** trigger upstream dependency updates.
...
### `get_function_code_summary(node_data)`
...
2.  If the new hash matches the `db_code_hash` and a `db_codeSummary` exists, it returns `unchanged`.
3.  If the DB is stale, it checks the cache for an entry matching the **new** hash. If found, it returns `code_summary_restored`.
4.  If both fail, it generates a new `codeSummary` via LLM and returns `code_summary_regenerated`, or `generation_failed` if the LLM call fails.
...
*   **Mechanism**: Before making an LLM call, the processor first calculates the total token count of the context. If it's over the limit, it uses helper methods (`_summarize_function_text_iteratively`, `_summarize_relations_iteratively`) to chunk the context and process it sequentially, feeding the output of one LLM call as input to the next.
