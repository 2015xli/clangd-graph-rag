...
class SymbolProcessor:
    ...
    def _process_and_filter_symbols(self, symbols: Dict[str, Symbol], qualified_namespace_to_id: Dict[str, str]) -> Dict[str, List[Dict]]:
        ...
        for sym in tqdm(symbols.values(), desc=align_string("Grouping symbols by kind")):
    ...
    def ingest_symbols_and_relationships(self, symbol_parser: SymbolParser, neo4j_mgr: Neo4jManager, defines_generation_strategy: str = "batched-parallel"):
        ...
        # NOTE: This is only needed for graph updater.
        ...
        for symbol_list in processed_symbols.values():
            for symbol_data in symbol_list:
                ...
                if "namespace_id" in symbol_data:
                    ...
                    is_valid_child = child_label in ('NAMESPACE', 'CLASS_STRUCTURE', 'DATA_STRUCTURE', 'FUNCTION', 'VARIABLE')

                    if is_valid_child:
                        scope_relation_key = (parent_label, child_label)
                        grouped_scope_relations[scope_relation_key].append({
                            "parent_id": parent_id,
    ...
    def _ingest_function_nodes(self, function_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        for i in tqdm(range(0, len(function_data_list), self.ingest_batch_size), desc=align_string("Ingesting FUNCTION nodes")):
    ...
    def _ingest_data_structure_nodes(self, data_structure_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        total_nodes_created = 0
        total_properties_set = 0
        for i in tqdm(range(0, len(data_structure_data_list), self.ingest_batch_size), desc=align_string("Ingesting DATA_STRUCTURE nodes")):
    ...
    def _ingest_field_nodes(self, field_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        total_nodes_created = 0
        total_properties_set = 0
        for i in tqdm(range(0, len(field_data_list), self.ingest_batch_size), desc=align_string("Ingesting FIELD nodes")):
    ...
    def _ingest_has_field_relationships(self, field_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        for i in tqdm(range(0, len(field_data_list), self.ingest_batch_size), desc=align_string("Ingesting HAS_FIELD relationships")):
            batch = field_data_list[i:i + self.ingest_batch_size]
    ...
    def _ingest_class_nodes(self, class_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        total_nodes_created = 0
        total_properties_set = 0
        for i in tqdm(range(0, len(class_data_list), self.ingest_batch_size), desc= align_string("Ingesting CLASS_STRUCTURE nodes")):
    ...
    def _dedup_nodes(self, neo4j_mgr: Neo4jManager):
        logger.info("Deduping DATA_STRUCTURE nodes if nodes of CLASS_STRUCTURE with same ids exist")
        query = """
        MATCH(ds: DATA_STRUCTURE)
        MATCH(cs: CLASS_STRUCTURE)
        WHERE ds.id = cs.id
        DETACH DELETE ds
        """
        counters = neo4j_mgr.execute_autocommit_query(query)
        logger.info(f"Total DATA_STRUCTURE nodes deleted: {counters.nodes_deleted}")
    ...
    def _ingest_method_nodes(self, method_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        total_nodes_created = 0
        total_properties_set = 0
        for i in tqdm(range(0, len(method_data_list), self.ingest_batch_size), desc=align_string("Ingesting METHOD nodes")):
    ...
    def _ingest_variable_nodes(self, variable_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        total_nodes_created = 0
        total_properties_set = 0
        for i in tqdm(range(0, len(variable_data_list), self.ingest_batch_size), desc=align_string("Ingesting VARIABLE nodes")):
    ...
    def _ingest_has_method_relationships(self, method_data_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        """
        total_rels_created = 0
        for i in tqdm(range(0, len(method_data_list), self.ingest_batch_size), desc=align_string("Ingesting HAS_METHOD relationships")):
            batch = method_data_list[i:i + self.ingest_batch_size]
    ...
    def _ingest_defines_relationships_batched_parallel(self, defines_function_list: List[Dict], defines_variable_list: List[Dict], defines_data_structure_list: List[Dict], defines_class_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        if defines_function_list:
            logger.info(f"  Ingesting {len(defines_function_list)} (FILE) -[:DEFINES]-> (FUNCTION) in batches (1 batch = {self.ingest_batch_size})")

            for i in tqdm(range(0, len(defines_function_list), self.ingest_batch_size), desc=align_string("Ingesting DEFINES relationships")):
            ...
            logger.info(f"  Total (FILE) -[:DEFINES]-> (FUNCTION) relationships created: {total_rels_created}, merged: {total_rels_merged}")

        # Ingest DATA_STRUCTURE DEFINES relationships
        total_rels_created = 0
        total_rels_merged = 0
        if defines_data_structure_list:
            logger.info(f"  Ingesting {len(defines_data_structure_list)} (FILE) -[:DEFINES]-> (DATA_STRUCTURE) in batches (1 batch = {self.ingest_batch_size})")
            for i in tqdm(range(0, len(defines_data_structure_list), self.ingest_batch_size), desc=align_string("Ingesting DEFINES relationships")):
                ...
                if results and len(results) > 0:
                    total_rels_created += results[0].get("totalRelsCreated", 0)
                    total_rels_merged += results[0].get("totalRelsMerged", 0)
            logger.info(f"  Total (FILE) -[:DEFINES]-> (DATA_STRUCTURE) relationships created: {total_rels_created}, merged: {total_rels_merged}")

        # Ingest CLASS_STRUCTURE DEFINES relationships
        total_rels_created = 0
        total_rels_merged = 0
        if defines_class_list:
            logger.info(f"  Ingesting {len(defines_class_list)} (FILE) -[:DEFINES]-> (CLASS_STRUCTURE) in batches (1 batch = {self.ingest_batch_size})")
            for i in tqdm(range(0, len(defines_class_list), self.ingest_batch_size), desc=align_string("Ingesting DEFINES relationships")):
                ...
                if results and len(results) > 0:
                    total_rels_created += results[0].get("totalRelsCreated", 0)
                    total_rels_merged += results[0].get("totalRelsMerged", 0)
            logger.info(f"  Total (FILE) -[:DEFINES]-> (CLASS_STRUCTURE) relationships created: {total_rels_created}, merged: {total_rels_merged}")

        # Ingest VARIABLE DEFINES relationships
        total_rels_created = 0
        total_rels_merged = 0
        if defines_variable_list:
            logger.info(f"  Ingesting {len(defines_variable_list)} (FILE) -[:DEFINES]-> (VARIABLE) in batches (1 batch = {self.ingest_batch_size})")
            for i in tqdm(range(0, len(defines_variable_list), self.ingest_batch_size), desc=align_string("Ingesting DEFINES relationships")):
                ...
                if results and len(results) > 0:
                    total_rels_created += results[0].get("totalRelsCreated", 0)
                    total_rels_merged += results[0].get("totalRelsMerged", 0)
            logger.info(f"  Total (FILE) -[:DEFINES]-> (VARIABLE) relationships created: {total_rels_created}, merged: {total_rels_merged}")
    ...
    def _ingest_defines_relationships_isolated_parallel(self, defines_function_list: List[Dict], defines_variable_list: List[Dict], defines_data_structure_list: List[Dict], defines_class_list: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        if defines_function_list:
            logger.info(f"  Ingesting {len(defines_function_list)} (FILE) -[:DEFINES]-> (FUNCTION) relationships...")
        ...
        if defines_data_structure_list:
            logger.info(f"  Ingesting {len(defines_data_structure_list)} (FILE) -[:DEFINES]-> (DATA_STRUCTURE) relationships...")
        ...
        if defines_class_list:
            logger.info(f"  Ingesting {len(defines_class_list)} (FILE) -[:DEFINES]-> (CLASS_STRUCTURE) relationships...")
        ...
        if defines_variable_list:
            logger.info(f"  Ingesting {len(defines_variable_list)} (FILE) -[:DEFINES]-> (VARIABLE) relationships...")
    ...
    def _ingest_inheritance_relationships(self, inheritance_relations: List[Tuple[str, str]], neo4j_mgr: Neo4jManager):
        ...
        total_rels_created = 0
        for i in tqdm(range(0, len(relations_data), self.ingest_batch_size), desc=align_string("Ingesting INHERITS relationships")):
    ...
    def _ingest_override_relationships(self, override_relations: List[Tuple[str, str]], neo4j_mgr: Neo4jManager):
        ...
        total_rels_created = 0
        for i in tqdm(range(0, len(relations_data), self.ingest_batch_size), desc=align_string("Ingesting OVERRIDDEN_BY relationships")):
            batch = relations_data[i:i + self.ingest_batch_size]
            counters = neo4j_mgr.execute_autocommit_query(query, {"relations": batch})
            total_rels_created += counters.relationships_created
    ...
    def _ingest_scope_contains_relationships(self, scope_relations: List[Dict], neo4j_mgr: Neo4jManager):
        ...
        logger.info(f"Creating {len(scope_relations)} SCOPE_CONTAINS relationships...")
        total_rels_created = 0
        for (parent_label, child_label), relations in scope_relations.items():
            logger.info(f"  Ingesting {len(relations)} SCOPE_CONTAINS relationships for ({parent_label})->({child_label})")
            query = f"""
            UNWIND $relations AS rel
            ...
            """
            for i in tqdm(range(0, len(relations), self.ingest_batch_size), desc=align_string("Ingesting SCOPE_CONTAINS relationships")):
    ...
    def _ingest_nesting_relationships(self, grouped_relations: Dict[Tuple[str, str], List[Dict]], neo4j_mgr: Neo4jManager):
        ...
        for (parent_label, child_label), relations in grouped_relations.items():
            ...
            """
            for i in tqdm(range(0, len(relations), self.ingest_batch_size), desc=align_string("Ingesting HAS_NESTED relationships")):
                batch = relations[i:i + self.ingest_batch_size]
                counters = neo4j_mgr.execute_autocommit_query(query, {"relations": batch})
                total_rels_created += counters.relationships_created
...
class PathProcessor:
    ...
    def _discover_paths_from_includes(self, compilation_manager: CompilationManager) -> set:
        ...
        for including_abs, included_abs in tqdm(include_relations, desc=align_string("Discovering paths from includes")):
